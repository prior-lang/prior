...
| `[morning_star]` / `[evening_star]` | predicate | — | 3-bar reversal: bar1 red (green), bar2 body smaller than bar1's with its body top (bottom) beyond bar1's close, bar3 green (red) closing past bar1's body midpoint. No gap requirement — daily stock bars rarely gap. No trend precondition — compose context as a sequence: `[down_days 3] then within 2 bars [morning_star]` (an and on the same bar would fight the pattern's own closing direction) |
