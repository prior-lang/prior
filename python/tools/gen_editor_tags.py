...
DESCRIPTIONS: dict[str, tuple[str, str]] = {
    ...
    "sma": ("Simple moving average of closes. Price compares with above/below, crosses above/below (an event: previous close on the other side), or at (the bar's range touched the line).", "when price crosses above [sma 50]"),
    "ema": ("Exponential moving average of closes. Price crosses/touches it like [sma]; in MA-vs-MA crosses the faster average goes on the left.", "when price crosses above [ema 9]"),
    "vwap": ("Rolling volume-weighted average price. Price compares, crosses, or touches it (at). Needs volume — unavailable on spreads.", "when price crosses above [vwap]"),
    ...
    "fractal_high": "The most recent confirmed fractal high as a price level. A bar whose high strictly beats the wing bars on each side; revealed wing bars after it forms, never on its own bar.",
    "fractal_low": "The most recent confirmed fractal low as a price level. A bar whose low strictly beats the wing bars on each side; revealed wing bars after it forms, never on its own bar.",
    ...
    "inside_bar": ("This bar's range sits strictly inside the prior bar's range.", "when [inside_bar]"),
    "outside_bar": ("This bar's range strictly engulfs the prior bar's range.", "when [outside_bar]"),
    "bullish_engulfing": ("A green body strictly engulfs the prior red body.", "when [bullish_engulfing]"),
    "bearish_engulfing": ("A red body strictly engulfs the prior green body.", "when [bearish_engulfing]"),
    "bullish_harami": ("The body sits strictly inside the prior red body.", "when [bullish_harami]"),
    "bearish_harami": ("The body sits strictly inside the prior green body.", "when [bearish_harami]"),
    "doji": ("The body is at most `body` percent of the bar's high-low range.", "when [doji 10%]"),
    "hammer": ("Lower shadow at least twice the body; upper shadow no larger than it.", "when [hammer]"),
    "shooting_star": ("Upper shadow at least twice the body; lower shadow no larger than it.", "when [shooting_star]"),
    "morning_star": ("Red bar, smaller body below its close, then a green close above the first bar's midpoint.", "when [morning_star]"),
    "evening_star": ("Green bar, smaller body above its close, then a red close below the first bar's midpoint.", "when [evening_star]"),
